...
#Correlation table
fxdata %>% select(btcret, gbpret, jpyret, eurret, cnyret, goldret) %>% cor(use = 'complete.obs')
...
ggplot(fxdata3a, aes(x=curr)) + geom_boxplot(aes(y=ret)) + ylim(-5, 5)
rm(fxdata3a)
...
#Testing of means
t.test(fxdata2$gbpret, fxdata2$eurret)
t.test(fxdata2$gbpret, fxdata2$jpyret)
t.test(fxdata2$eurret, fxdata2$jpyret)
t.test(fxdata2$cnyret, fxdata2$jpyret)

t.test(fxdata2$btcret, fxdata2$eurret)
t.test(fxdata2$btcret, fxdata2$jpyret)
t.test(fxdata2$btcret, fxdata2$gbpret)
t.test(fxdata2$btcret, fxdata2$cnyret)
t.test(fxdata2$btcret, fxdata2$goldret)

#Similarly do a var.test(): However note that even between currencies, variance equality tests would show variances are not equal
#Thus this test may not mean very much
var.test(fxdata2$btcret, fxdata2$eurret)
var.test(fxdata2$btcret, fxdata2$jpyret)
var.test(fxdata2$btcret, fxdata2$gbpret)
var.test(fxdata2$btcret, fxdata2$cnyret)
var.test(fxdata2$btcret, fxdata2$goldret)
