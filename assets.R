...
fxdata3b <- fxdata %>% select(Date, btcret, gbpret, jpyret, eurret, goldret) %>% gather(curr, ret, -Date, -jpyret)
ggplot(fxdata3b, aes(x = jpyret)) + geom_point(aes(y=ret)) + facet_grid(~curr) + xlim(-5, 5) + ylim(-5, 5) +
  theme_Publication() +
