...
Mining difficulty is almost perfectly correlated to bitcoin price, with a correlation coefficient of 0.9009963, while supply of bitcoin is still strong with a coefficient of 0.526925. Given the strength of the mining coefficient, we can partly say that plays a key component in it's overall supply function.
...
We will now examine further examine Bitcoin's supply and demand mechanics using variables such as transactions, prices and coin supply in tandem. We will build an econometric model that will identify what mechanics drive Bitcoin's price, in other words is it supply or demand based driven. If it is more demand based driven via transactions and if the effect size is large then we can imply that it is versatile as a medium of exchange. The coefficients from our model will also indicate the magnitude and direction of effects.
...
When Bitcoin supply increases by 1% (ceteris paribus), the Exchange Price decreases by 22.14%. In contrast, when transactions increase by 1% (ceteris paribus), the Exchange Price increases by 4.15%. Both suppy and transactions are significant at the 5% level. 
